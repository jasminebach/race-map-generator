...
st.set_page_config(page_title="FSAE Lap Simulator", layout="wide")
...
st.set_page_config(page_title="Lap Simulator + Metrics", layout="wide")
st.title("Track + Dynamic Driver Simulator + Metrics")
...
randomize = st.sidebar.checkbox('Randomize track generation', value=True)
...
uploaded = st.sidebar.file_uploader('Upload simulated CSV (optional)', type=['csv'])
...
if simulate_btn:
    ...
    # use previous run if CSV uploaded
    if uploaded is not None:
        df_sim = pd.read_csv(uploaded, encoding="utf-8")
        st.success('Using uploaded simulated run.')
    else:
        if skill_level == 'Beginner':
            aggr = 0.8; s_noise = 3.0; t_noise = 0.6
        elif skill_level == 'Intermediate':
            aggr = 1.0; s_noise = 1.2; t_noise = 0.3
        else:
            aggr = 1.15; s_noise = 0.5; t_noise = 0.12

        df_sim = simulate_bicycle(
            Xc, Yc, vmax, dt=dt, wheelbase=wheelbase, mass=mass,
            max_steer=np.deg2rad(max_steer_deg),
            max_acc=max_acc, max_brake=max_brake,
        ...
        st.info(f'Saved run: `{fname}`')


        out_folder2 = "centerline"
        os.makedirs(out_folder2, exist_ok=True)
        
        centerline_df = pd.DataFrame({"x": Xc, "y": Yc})
        centerline_path = os.path.join(out_folder2, f"centerline_{int(time.time())}.csv")

        centerline_df.to_csv(centerline_path, index=False)
        st.info(f"Saved centerline: `{centerline_path}`")
    ...
    st.write("Apex miss:", rate(metrics['Apex miss (m)'], 0.8, 1.5))
    st.write("Slalom RMS:", rate(metrics['Slalom RMS (m)'], 1.2, 2.0))
    st.write("Throttle Aggression:", rate(metrics['Throttle Aggression (m/s^2)'], 2.0, 3.5))
